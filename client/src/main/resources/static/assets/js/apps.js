...
var handleThemePageStructureControl = function() {
	...
	$(document).on('click', '.theme-list [data-theme]', function() {
		var cssFileSrc = $(this).attr('data-theme-userFile');
	...
	function renderAjax(url, elm, disablePushState) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				swal(webAlert({text:"登录超时或页面异常，请重新登录！",icon:"warning",getArg: true}))
					.then(function(){
						var url = document.location.toString();
						var arrUrl = url.split("//");
						var start = arrUrl[1].indexOf("/");
						var relUrl = arrUrl[1].substring(start);
						location.href="/login?url="+relUrl;
					});
				};
			...
			error: function(jqXHR, textStatus, errorThrown) {
				if(jqXHR.status==404){
					webAlert("页面没找到","404 页面没找到，通过以下方式排除问题：\n\n1：请确认地址是否正确。\n2：浏览其它页面。");
				}else if(jqXHR.readyState==0 && jqXHR.status==0){
					webAlert("无法连接至服务器","请按以下方式排除问题：\n\n1：请检查网络是否正常;\n2：确认是否正在维护服务器；\n3：请稍候再试。");
				}
